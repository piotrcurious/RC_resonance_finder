...
#define PIN_OUT 11 // digital output pin (changed from 13 to 11)
#define PIN_IN A0 // analog input pin
#define C 10e-6 // capacitance of RC circuit in farads
#define C0 0.5e-6 // capacitance of output pin in farads (changed from 0.1e-6 to 0.5e-6)
...
P[i][i] +
Rn[i][i]); // K[i][j] =
P[i][j]*H[j][i]/(H[i][i]*P[i][i]+Rn[i][i])
    }
  }
...
void setup() {
  ...
  // Change the timer prescaler value for timer 2 to increase the PWM frequency for pins 3 and 11
  TCCR2B = TCCR2B & B11111000 | B00000001; // set the prescaler to 1
}

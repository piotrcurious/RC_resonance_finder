...
// Define a function to update the PID controller using the expected voltage value
void updatePID(float v1) {
  ...
  // Calculate the output value for pulse length adjustment
  output = KP * error + KI * error_sum + KD * error_diff;
}

// Define constants for relay feedback method
#define RELAY_HIGH 0.1 // High value of relay signal in seconds
#define RELAY_LOW -0.1 // Low value of relay signal in seconds
#define RELAY_HYSTERESIS 0.01 // Hysteresis value of relay signal in volts
#define RELAY_TIMEOUT 10 // Timeout value for relay signal in seconds

// Define variables for relay feedback method
float relay; // Current value of relay signal
float relay_prev; // Previous value of relay signal
float v1; // Current voltage across the first capacitor
float v1_prev; // Previous voltage across the first capacitor
float t1; // Current time
float t1_prev; // Previous time
float t_high; // Time when relay signal is high
float t_low; // Time when relay signal is low
float a_high; // Amplitude of oscillation when relay signal is high
float a_low; // Amplitude of oscillation when relay signal is low
float ku; // Ultimate gain of PID controller
float tu; // Ultimate period of PID controller

// Define a function to perform the relay feedback method and return the optimal PID controller parameters
float* pidAutoTune() {
  // Initialize the relay signal to high value
  relay = RELAY_HIGH;
  relay_prev = RELAY_HIGH;

  // Initialize the voltage and time values to zero
  v1 = 0;
  v1_prev = 0;
  t1 = 0;
  t1_prev = 0;

  // Initialize the time and amplitude values to zero
  t_high = 0;
  t_low = 0;
  a_high = 0;
  a_low = 0;

  // Initialize the ultimate gain and period values to zero
  ku = 0;
  tu = 0;

  // Repeat until timeout or oscillation is detected
  while (t1 - t1_prev < RELAY_TIMEOUT * 1000000 && (t_high == 0 || t_low == 0)) {
    // Send the relay signal to the output pin
    digitalWrite(OUT_PIN, HIGH);
    delayMicroseconds(relay * 1000000);
    digitalWrite(OUT_PIN, LOW);

    // Measure the voltage across the first capacitor
    v1 = analogRead(IN_PIN) * VCC / 1023;

    // Update the current time value
    t1 = micros();

    // Check if the voltage crosses the hysteresis band around the relay signal value
    if (v1 > relay + RELAY_HYSTERESIS && v1_prev < relay + RELAY_HYSTERESIS) {
      // Switch the relay signal to low value
      relay = RELAY_LOW;

      // Record the time and amplitude when the relay signal is high
      t_high = t1 - t1_prev;
      a_high = v1 - v1_prev;
    }
    else if (v1 < relay - RELAY_HYSTERESIS && v1_prev > relay - RELAY_HYSTERESIS) {
      // Switch the relay signal to high value
      relay = RELAY_HIGH;

      // Record the time and amplitude when the relay signal is low
      t_low = t1 - t1_prev;
      a_low = v1 - v1_prev;
    }

    // Update the previous voltage and time values
    v1_prev = v1;
    t1_prev = t1;
  }

  // Calculate the ultimate gain and period of the PID controller using the time and amplitude values
  ku = (4 * RELAY_HIGH) / (PI * (a_high + a_low));
  tu = t_high + t_low;

  // Calculate the optimal PID controller parameters using some formulas based on the Ziegler-Nichols method
  float kp = 0.6 * ku;
  float ki = 1.2 * ku / tu;
  float kd = 0.075 * ku * tu;

  // Create an array to store and return the PID controller parameters
  static float pid[3];
  pid[0] = kp;
  pid[1] = ki;
  pid[2] = kd;

  // Return the array of PID controller parameters
  return pid;
}

// Define a function to find the value of the unknown resistor R2
float findR2() {
...
void loop() {
  // Call the pidAutoTune function and print the optimal PID controller parameters to serial monitor
  float* pid = pidAutoTune();
  Serial.print("KP: ");
  ...
  // Call the findR2 function using the optimal PID controller parameters and print the result to serial monitor
  float r2 = findR2(pid[0], pid[1], pid[2]);
  Serial.print("R2: ");
  Serial.println(r2);
  Serial.println();

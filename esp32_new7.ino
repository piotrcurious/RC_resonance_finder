...
// ============================================================
// Dual-node ESP32 RC estimator with runtime ADC calibration
// ------------------------------------------------------------
// New in this version:
//   - Per-node runtime zero-offset calibration
//   - Calibrated mV reads via analogReadMilliVolts()
//   - Adaptive noise estimation per node
//   - Better fallback if stage 1 is too fast/noisy
//   - Median of multiple capture runs
//
// Physical model:
...
// Sample positions for each capture
...
static float readVoltageAvgMv(int pin, uint8_t samples = 16) {
  // analogReadMilliVolts() is the calibrated conversion path on Arduino-ESP32.
  // We still average several reads because the ADC and the wiring are noisy.
  (void)analogReadMilliVolts(pin); // flush first read
...
// ----------------------------
// ADC runtime calibration
// ----------------------------
struct AdcNodeCal {
  bool valid = false;
  float zeroOffsetMv = 0.0f;
  float noiseMv = 5.0f;
  uint16_t lastBaselineMv = 0;
};

static AdcNodeCal cal1;
static AdcNodeCal cal2;

static float correctedMv(int pin, const AdcNodeCal &cal) {
  float mv = readVoltageAvgMv(pin, 16);
  mv -= cal.zeroOffsetMv;
  if (mv < 0.0f) mv = 0.0f;
  return mv;
}

static AdcNodeCal calibrateNodeZero(int pin, float dischargeTauUs) {
  AdcNodeCal cal;
  const uint8_t rounds = 12;

  // Make sure the node is fully discharged first.
  digitalWrite(OUT_PIN, LOW);
  waitMicros(clampu32((uint32_t)(dischargeTauUs * 6.0f), 5000UL, 2000000UL));
  ...
  float copy[rounds];
  for (uint8_t i = 0; i < rounds; ++i) copy[i] = samples[i];
  float med = medianOfArray(copy, rounds);

  // Robust spread estimate around median.
  float mad = 0.0f;
  ...
  cal.lastBaselineMv = (uint16_t)clampf(med, 0.0f, 65535.0f);
  return cal;
}

static void refreshCalibration(bool forcePrint = false) {
  // Stage 1 node is fast, so calibrate its zero using a short discharge window.
...
// Capture helpers
// ----------------------------
static FitResult captureOneNode(int pin, const AdcNodeCal &cal, float seedTauUs, uint32_t minPulseUs, uint32_t maxPulseUs) {
  ...
  float v0 = correctedMv(pin, cal) / 1000.0f;
  ...
  for (uint8_t i = 0; i < SAMPLE_COUNT; ++i) {
    ...
    vVolts[i] = correctedMv(pin, cal) / 1000.0f;
  }
...
static FitResult bestOfSeveral(int pin, const AdcNodeCal &cal, float seedTauUs, uint32_t minPulseUs, uint32_t maxPulseUs) {
...
static float medianTauAcrossCaptures(int pin, const AdcNodeCal &cal, float seedTauUs, uint32_t minPulseUs, uint32_t maxPulseUs, float &bestSigma, float &bestR2, uint8_t &bestPoints) {
...
static void runResistanceCheck() {
  ...
  // Refresh the runtime ADC zero calibration each cycle.
  // This captures slow offset drift and residual node leakage.
  ...
  uint8_t s1Pts = 0;
  float tau1Med = medianTauAcrossCaptures(SENSE1_PIN, cal1, stage1SeedUs, 20, 5000, s1Sigma, s1R2, s1Pts);
  ...
  uint8_t s2Pts = 0;
  float tau2Med = medianTauAcrossCaptures(SENSE2_PIN, cal2, stage2SeedUs, 500, 2000000, s2Sigma, s2R2, s2Pts);
